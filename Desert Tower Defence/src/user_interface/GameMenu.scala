 ...
 *  Takes care of all the clicking, both in the menu and on the arena,
 *  drawing the store, the store menu and the infoScreen.*/
class GameMenu(g: Game) extends Helper(g) {
  ...
  def doStuff() = {
    ...
    g.image(g.menuS(0),sqSize*aWidth,0,sqSize*mWidth,sqSize*aHeight)
    
    // draws the players hp and money stats
    g.fill(0) 
    g.textFont(g.font,20)
    g.text(" Money:   " + player.money,sqSize*aWidth+ 9,51)
    g.fill(255, 255, 0)
    g.text(" Money:   " + player.money,sqSize*aWidth+ 8,50)
    g.fill(0) 
    g.textFont(g.font,22)
    g.text("   HP:    " + player.hp,sqSize*aWidth+ 9,81)
    g.fill(255, 0, 0)
    g.text("   HP:    " + player.hp,sqSize*aWidth+ 8,80)
  ...
  def drawMuteButton() = {
    if (g.sounds.muted)
      g.image(g.muteButton(1),muteLoc._1,muteLoc._2)
    else
      g.image(g.muteButton(0),muteLoc._1,muteLoc._2)
  }
  ...
  def drawWaveButton() = {
    ...
    def draw() = {
      g.rect(waveButtonPos._1,waveButtonPos._2,waveButtonSize._1,waveButtonSize._2)
      g.textFont(g.font,12)
      g.fill(0) 
      g.text(text,waveButtonPos._1 + sqSize*3/4 +1,waveButtonPos._2 + sqSize*7/8 + 1)
      g.fill(250) 
      g.text(text,waveButtonPos._1 + sqSize*3/4,waveButtonPos._2 + sqSize*7/8)
    }
    
    if (onWaveButton) {
      g.fill(color._1,color._2,color._3,160)
      draw()
    } else {
      g.fill(color._1,color._2,color._3,100)
      draw()
    }
  }
  /////////////////////////////////////////////////////////////////////////////////////////
  ...
  def drawFastButton() = {
    ...
    def draw() = {
      g.rect(fastButtonPos._1,fastButtonPos._2,fastButtonSize._1,fastButtonSize._2)
      g.textFont(g.font,12)
      g.fill(0) 
      g.text(text,fastButtonPos._1 + sqSize +1,fastButtonPos._2 + sqSize*5/8 + 1)
      g.fill(250) 
      g.text(text,fastButtonPos._1 + sqSize,fastButtonPos._2 + sqSize*5/8)
    }
  ...
  def drawMenuButton() = {
    ...
    // A bit wetwet, but there are some details that make it difficult to generalize 
    def draw() = {
      g.rect(menuButtonPos._1,menuButtonPos._2,menuButtonSize._1,menuButtonSize._2)
      g.textFont(g.font,12)
      g.fill(0) 
      g.text(text,menuButtonPos._1 + sqSize +1,menuButtonPos._2 + sqSize*5/8 + 1)
      g.fill(250) 
      g.text(text,menuButtonPos._1 + sqSize,menuButtonPos._2 + sqSize*5/8)
    }
  ...
  /**Returns the square which the mouse is on on the arena. */
  def mouseSq = {
  ...
  /** Determines what happens in the infoScreen when the mouse is on the store menu.*/
  def mouseStoreMenu() = {
  ...
  /** Determines what happens in the infoScreen when the mouse is over the arena. */
  def mouseArena() = {
    if (!onMenu) {
     val sq = mouseSq
     sq match {
